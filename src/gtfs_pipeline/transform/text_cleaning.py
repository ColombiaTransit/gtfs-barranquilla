...
def clean_text_field(value: str | None) -> str:
    ...
    spaces, strips control characters, and trims the result. Returns ""
    for None/empty input -- callers that require a non-empty value (e.g.
    stop_name) must check for that themselves; this function only cleans,
